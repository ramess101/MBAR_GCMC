...
compound_list = {'alkanes':['C6H14'],'C-CH-group':['223TMButane','223TMPentane','224TMPentane','233TMPentane'],'C-group':['3M3EPentane','22DMButane','22DMHexane','22DMPentane','22DMPropane','33DMHexane','33DMPentane','2233TetraMButane'],'CH-group':['2M3EPentane','2MButane','2MHeptane','2MHexane','2MPentane','2MPropane','3EHexane','3EPentane','3MHeptane','3MHexane','3MPentane','4MHeptane','23DMBUtane','23DMHexane','23DMPentane','24DMHexane','24DMPentane','25DMHexane','34DMHexane','234TMPentane'],'alkynes':['ethyne','propyne','1butyne','2butyne','1pentyne','2pentyne','1hexyne','2hexyne','1heptyne','1octyne','1nonyne']}  #{'C-group':['22DMPropane'],'C-CH-group':['224TMPentane']}
...
#        if (compound in REFPROP_list or group == 'alkynes') and reprocess:# and not os.path.exists('H:/MBAR_GCMC/figures/'+compound+'_AD_eps_scan.pdf'):
        ...
        Tsat_Potoff = np.array([330.,360.,390.,420.,450.])
        Tsat_RP = Tsat_Potoff.copy()
        Mw = 12.0107*6.+1.0079*14.
        
        Tsat_RP = np.array([330.,360.,390.,420.,450.,480.])
        rhol_RP = np.array([624.97,594.94,562.07,524.81,479.97,418.49])
        rhov_RP = np.array([2.2413,5.3690,11.227,21.489,39.332,73.332]) 
        Psat_RP = np.array([0.68708,1.7327,3.7185,7.0804,12.329,20.093])
        ...
        rhol_model = MBAR_GCMC_trial.rholiq
        rhov_model = MBAR_GCMC_trial.rhovap
        Psat_model = MBAR_GCMC_trial.Psat
        MBAR_GCMC_trial.solve_VLE(Tsat_Potoff,1.5)
        MBAR_GCMC_trial.plot_VLE([],[],[],[],[],[])
        rhol_rr_model = MBAR_GCMC_trial.rholiq
        rhov_rr_model = MBAR_GCMC_trial.rhovap
        Psat_rr_model = MBAR_GCMC_trial.Psat
        
        plt.plot(rhol_model,Tsat_Potoff,'ro')
        plt.plot(rhov_model,Tsat_Potoff,'ro')
        plt.plot(rhol_rr_model,Tsat_Potoff,'bs')
        plt.plot(rhov_rr_model,Tsat_Potoff,'bs')
        plt.plot(rhol_RP,Tsat_RP,'k-')
        plt.plot(rhov_RP,Tsat_RP,'k-')
        plt.show()
        
        plt.plot(1000./Tsat_Potoff,np.log10(Psat_model),'ro')
        plt.plot(1000./Tsat_Potoff,np.log10(Psat_rr_model),'bs')
        plt.plot(1000./Tsat_RP,np.log10(Psat_RP),'k-')
        plt.show()
